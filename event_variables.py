...
class EventVariablesFrame(tk.Frame):
    def __init__(self, parent, callbacks, node, event_variables, *args, **kwargs):
        ...
        for i in range(self.node['number_of_event_variables'] + 1):
            self.enter_event_variable(i, 0, self.event_variables[i])
        # enter_node_variable(new_window, 2, 0, self.selected_node_variables[2])

    def enter_event_variable(self, row, column, variable):
